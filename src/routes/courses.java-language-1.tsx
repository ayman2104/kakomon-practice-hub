...
export const Route = createFileRoute("/courses/java-language-1")({
  head: () => ({ meta: [{ title: "Java言語Ⅰ | Kakomon Practice Hub" }] }),
...
function CoursePage() {
  ...
  const isChildPage = pathname !== "/courses/java-language-1";
    ...
    <StudentShell>
      <div className="space-y-6">
        <div className="bg-card rounded-3xl shadow-[var(--shadow-card)] p-6 sm:p-8">
          ...
          <h1 className="text-2xl font-bold mt-1">Java言語Ⅰ</h1>
          <p className="text-sm text-muted-foreground mt-3 leading-relaxed">
            Java言語Ⅰの基本文法、条件分岐、配列、クラス、継承、カプセル化、抽象クラスをランダム問題で練習します。
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <Link
            to="/courses/java-language-1/practice"
